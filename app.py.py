...
# ============================================================
# SUPABASE CLIENT + DETERMINISTIC PKCE OAUTH
# ============================================================

# Streamlit's link button opens Google in a new browser session. A verifier
# kept only in process/session memory can therefore disappear before the
# callback arrives (or after an app restart).
#
# We instead derive the PKCE verifier deterministically from the OAuth state
# value and a server-side secret. The state is returned by Supabase with the
# callback, so no browser cookie, Streamlit session, or process-local store is
# required. The verifier itself is never sent to Google; only its challenge is.

GOOGLE_OAUTH_SECRET = hashlib.sha256(
    f"{SUPABASE_KEY}|{GROQ_API_KEY}".encode("utf-8")
).digest()


def _derive_google_pkce_verifier(state):
    """Derive one stable PKCE verifier from the returned OAuth state."""
    if not state:
...
with st.sidebar:
    ...
    st.caption("✨ RacharlaGPT next-generation student UI")
...
# ============================================================
# NEXT-GENERATION STUDENT COMMAND CENTER — UI ONLY
# ============================================================

if not chat["messages"]:
    st.markdown("### 🚀 Start with a goal")
    st.caption("Choose a starting point, or simply type your own question below.")

    quick_actions = [
        ("📚 Learn", "Teach me an important topic I should learn next based on my current level."),
        ("💻 Coding", "Give me a practical coding challenge with a hint first."),
        ("📊 Data Analyst", "Give me a practical data analytics task using SQL, Excel, Python, or Power BI."),
        ("🧪 Practice", "Quiz me with 5 questions on an important skill and wait for my answers."),
        ("🎯 Interview", "Start a realistic technical interview for a beginner-to-intermediate data/technology role."),
        ("🏗️ Project", "Give me a portfolio-worthy real-world project with requirements and milestones."),
    ]

    cols = st.columns(3)
    for i, (label, prompt) in enumerate(quick_actions):
        with cols[i % 3]:
            if st.button(label, key=f"quick_start_{i}", use_container_width=True):
                st.session_state["quick_prompt"] = prompt
                st.rerun()

    with st.expander("🧭 What should I learn next?", expanded=False):
        st.markdown(
            """
**Recommended growth path**

1. **Foundation** — computer basics, problem solving, Excel, SQL
2. **Core technical skill** — Python or JavaScript
3. **Analytics** — data cleaning, statistics, Power BI, dashboards
4. **AI** — prompting, AI-assisted coding, data/automation workflows
5. **Projects** — build 2–4 practical portfolio projects
6. **Career** — GitHub, resume, portfolio, interview practice

Your exact path should depend on your current skills, target role, and available study time.
"""
        )
...
# A quick-start button can safely hand its prompt to the same normal message pipeline.
